"""
the profitability service is responsible for calculating the profitability of a project

service should perform create and get operations for the project_profitability_snapshots table in supabase

the shape of a snapshot is as follows:

create table public.project_profitability_snapshots (
  id uuid not null default gen_random_uuid (),
  created_at timestamp with time zone not null default now(),
  project_id uuid not null default gen_random_uuid (),
  baseline_id uuid null default gen_random_uuid (),
  total_profitability double precision not null,
  triggered_by_agent character varying null,
  triggered_by_action character varying null,
  constraint project_profitability_snapshots_pkey primary key (id),
  constraint project_profitability_snapshots_project_id_fkey foreign KEY (project_id) references projects (project_id)
) TABLESPACE pg_default;

baseline_id is a foreign key to the project_profitability_baselines table
if baseline_id is null, then the snapshot is a baseline

to calculate total_profitability, we need to run this sql code
...
get_profitability_snapshot_by_id(snapshot_id: uuid) -> SnapshotResponse:
create_snapshot_from_project_id(project_id: uuid, baseline_id: uuid | None = None) -> SnapshotResponse:
get_latest_baseline_for_project(project_id: uuid) -> SnapshotResponse:
calculate_profitability_change_since_baseline(snapshot_id: uuid)
